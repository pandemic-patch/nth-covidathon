...
import { Label, Message, Card, Button, Icon, Grid } from 'semantic-ui-react';
var QRCode = require('qrcode.react');

// patient & physician
var keys = {
  patient: '0xBEDa8BE440d4afae4c4E28469fABd24a20e1b747',
  physician: '0x66E437D3FB414Ca912678c7Ef49C7EDB6347cfdD',
};
    ...
    if (role === 'patient') {
      ...
      this.postData(apiUrl + 'api/TransactionHeaders', {
        ...
        timeElapsed: Math.round(
          (new Date().getTime() - self.state.startTime.getTime()) / 1000
        ),
        ...
        .then(data => {
          ...
          self.setState({
            showTimer: false,
            qrCodePayment: data['qrCode'],
            chargeDollar: data['charges'],
          });
        ...
        .catch(error => {
          ...
          self.setState({
            showTimer: false,
            showErrorMsg: true,
            errorMsg: 'We cannot process your payment. Please contact us',
          });
        });
    }
  }
            ...
            <Grid.Row>
              <Grid.Column tablet={3} computer={3} mobile={16}>
                <Card className='card_c_'>
                  ...
                  <Card.Content>
                    You're a
                    <Label as='a' className='txt_label'>
                      {this.state.clientId}
                    </Label>
                  </Card.Content>
                  <Card.Content extra>
                ...
                <Card className='card_c_'>
                  ...
                  {isPatient && (
                    <Card.Content>
                      {this.state.chargeDollar > 0 && (
                        <p> HMO {this.state.chargeDollar}</p>
                      )}
                      {this.state.qrCodePayment && (
                        <QRCode value={this.state.qrCodePayment} />
                      )}
